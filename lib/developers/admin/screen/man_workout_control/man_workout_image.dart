...
class ManWorkoutImage extends StatefulWidget {
  @override
  _ManWorkoutImageState createState() => _ManWorkoutImageState();
}

class _ManWorkoutImageState extends State<ManWorkoutImage> {
  final GlobalKey<FormState> _manWorkoutImageFormKey = GlobalKey();
  ...
  Future uploadFile() async {
    if (!_manWorkoutImageFormKey.currentState.validate()) {
      return;
    }
    _manWorkoutImageFormKey.currentState.save();

    try {
      ref = firebase_storage.FirebaseStorage.instance
          .ref()
          .child('manworkout/${Path.basename(_image[_image.length - 1].path)}');
      var result =
  ...
  void initState() {
    super.initState();
    imgRef = FirebaseFirestore.instance.collection('manWorkoutImage');
  }
